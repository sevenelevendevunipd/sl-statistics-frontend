...
describe("FirmwareChartDataStore", () => {
  ...
  describe("update", () => {
    const postApiChartsFirmwareSpy = jest.spyOn(ChartsService, 'postApiChartsFirmware');

    it("updates successfully with all firmwares", done => {
      const mockData = { bars: [{ firmware: "v1", count: "10" }, { firmware: "v2", count: "20" }] };
      postApiChartsFirmwareSpy.mockResolvedValue(mockData);
      firmwareChartDataStore.update(new Date(), new Date(), ["v1", "v2"]);

      expect(firmwareChartDataStore.state).toEqual(FirmwareChartStoreState.waiting);
      ...
      setTimeout(() => {
        ...
        expect(firmwareChartDataStore.state).toEqual(FirmwareChartStoreState.idle);
        done();
      }, 0);
    });
    ...
    it("updates successfully with selected firmwares", done => {
      ...
      setTimeout(() => {
        expect(postApiChartsFirmwareSpy).toBeCalledWith(expect.objectContaining({
